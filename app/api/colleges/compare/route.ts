...
export async function GET(request: Request) {
  ...
  const [programs, reviews, scholarships] = await Promise.all([
    db.from('college_programs').select('college_id,fee,fee_period,fee_academic_year,seats,scholarship_available,program:programs(name,degree_level)').in('college_id', ids),
  ...
  const details = Object.fromEntries(ids.map(id => [id, {
    programs: (programs.data || []).filter(row => row.college_id === id).map(row => ({ fee: row.fee, fee_period: row.fee_period, fee_academic_year: row.fee_academic_year, seats: row.seats, scholarship_available: Boolean(row.scholarship_available), program: row.program })),
