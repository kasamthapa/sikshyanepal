...
export default async function ScholarshipsPage({ searchParams }: { searchParams: Params }) {
  ...
  return <main className="min-h-screen bg-[#f8f7f3]"><div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8"><div className="max-w-3xl"><p className="text-xs font-bold uppercase tracking-[.14em] text-[#9a302c]">Scholarships</p><h1 className="mt-2 font-display text-4xl font-bold text-ink">Scholarships in Nepal</h1><p className="mt-3 leading-7 text-slate-600">Filter by study level, eligibility and deadline. Confirm every requirement with the provider.</p></div>
    ...
    {!error&&schemaLimited&&<div role="status" className="mt-6 flex gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0"/><div><p className="font-bold">Scholarship matching is temporarily unavailable</p><p className="mt-1">Basic scholarship listings are still available. Check again later for matching tools.</p></div></div>}
    {!error&&<><div className="mt-8"><h2 className="text-xl font-bold text-ink">{hasFilters?'Matching opportunities':'Current opportunities'}</h2><p className="mt-1 text-sm text-slate-500">{items.length} scholarship{items.length===1?'':'s'} listed{status==='open'?' with an open or pending deadline':''}</p></div>
