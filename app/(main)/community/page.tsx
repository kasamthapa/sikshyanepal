...
export default async function CommunityPage({ searchParams }: { searchParams: { topic?: string; sort?: string } }) {
  ...
  return <main className="min-h-screen bg-[#f6f7fb]">
    <section className="border-b border-gray-200 bg-white"><div className="mx-auto max-w-6xl px-4 py-10 sm:px-6"><p className="text-xs font-bold uppercase tracking-widest text-primary">Student community</p><h1 className="mt-3 font-display text-3xl font-extrabold text-ink sm:text-4xl">Join with a public display name.</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-gray-600">Other students see your chosen public name. SikshyaNepal retains your sign-in details for safety and moderation. Every post and reply is reviewed before publication.</p><div className="mt-5 flex items-start gap-2 rounded-xl bg-amber-50 p-3 text-xs leading-5 text-amber-900"><ShieldCheck className="mt-0.5 h-4 w-4 flex-shrink-0" aria-hidden="true"/><span>Do not share contact details, precise addresses, threats, accusations or identifying details. This board is not an emergency service.</span></div></div></section>
    <div className="mx-auto grid max-w-6xl gap-7 px-4 py-8 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      <div className="min-w-0">
        ...
        {failed ? <div className="rounded-2xl border border-amber-200 bg-amber-50 p-8 text-center text-amber-900"><h2 className="font-bold">Community could not load</h2><p className="mt-2 text-sm">Please refresh the page or try again shortly.</p></div> : posts.length ? <div className="space-y-3">{posts.map(post => <article key={post.id} className="rounded-2xl border border-gray-200 bg-white p-5 transition hover:border-blue-200 hover:shadow-sm"><div className="flex flex-wrap items-center gap-2 text-xs"><span className="rounded-full bg-blue-50 px-2.5 py-1 font-bold text-blue-700">{topicLabel(post.topic)}</span><span className="text-gray-400">@{post.public_alias || 'LegacyStudent'} · {timeLabel(post.published_at || post.created_at)}</span></div><Link href={`/community/${post.id}`}><h2 className="mt-3 text-lg font-bold text-ink hover:text-primary">{post.title}</h2>{post.body && <p className="mt-2 line-clamp-3 text-sm leading-6 text-gray-600">{post.body}</p>}</Link><div className="mt-4"><CommunityMedia url={post.media_path ? `/api/community/media/${post.id}` : post.media_url} type={post.media_type} compact/></div><div className="mt-4 flex items-center justify-between"><VoteButtons targetType="post" targetId={post.id} initialScore={post.vote_score}/><p className="flex items-center gap-1.5 text-xs font-semibold text-gray-400"><MessageCircle className="h-4 w-4"/>{post.community_comments?.[0]?.count || 0} replies</p></div></article>)}</div> : <div className="rounded-2xl border border-dashed border-gray-300 bg-white p-12 text-center"><MessageCircle className="mx-auto h-10 w-10 text-gray-300"/><h2 className="mt-4 font-bold text-ink">No published discussions yet</h2><p className="mt-2 text-sm text-gray-500">Start a useful discussion. It will appear after moderation.</p></div>}
      </div>
