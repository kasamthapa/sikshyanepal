...
import { Bell, ExternalLink, Calendar, Sparkles, FileDown, ImageIcon } from 'lucide-react'
      ...
      <Link href={`/notices/${notice.slug}`} className="block group">
        ...
        >
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${accent.iconBg}`}>
            <Bell className={`w-3.5 h-3.5 ${accent.icon}`} />
          </div>
      ...
      >
        <div className="flex items-start gap-3">
          <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${accent.iconBg}`}>
            <Bell className={`w-5 h-5 ${accent.icon}`} />
          </div>
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <Badge variant={accent.badge}>{notice.university?.short_name}</Badge>
              {notice.university?.name && (
                <span className="text-xs text-ink-muted truncate max-w-[140px]">{notice.university.name}</span>
              )}
            </div>
